...
vec4 shadedCloudColour(vec3 pos, float h, float thickness, float turbulence) {
    ...
    : mix(0.6, 3.0, smoothstep(0.2, 1.0, h))
    ) * dayBrightness;
    
    
    float bottomness = pow(1.0 - h, 2.0);
    vec3 blueTint = vec3(0.5, 0.8, 1.0);

    vec3 greyTint = vec3(0.82, 0.9, 1.0);



    float blueStrength = min((h < 0.2
    ? mix(0.3, 0.5, smoothstep(0.0, 0.2, h))
    : 0.2
    ),0.5);
